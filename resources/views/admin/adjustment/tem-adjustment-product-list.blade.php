...
@empty
    ...
    <tr>
        ...
        <td colspan="1" class="text-left">{{$currency.' '.$subTotal}}
            <input type="hidden" name="sub_total" value="{{$subTotal}}" id="netTotalHidden" /></td>
    </tr>

    <tr>
        <td colspan="3" class="text-right"><strong>Discount :</strong></td>

        <td colspan="1" class="text-left">{{$currency}} <input type="number" name="discount" value="{{$discount}}" min="0" id="discount"> </td>
    </tr>

    <tr>
        <td colspan="3" class="text-right"><strong>Net Total :</strong></td>

        <td colspan="1" class="text-left">{{$currency}} <span id="netTotal"> {{$subTotal-$discount}} </span></td>

        <input type="hidden" name="net_total" value="{{$subTotal-$discount}}" readonly id="netTotalHidden" />
    </tr>
...
<script>
    ...
    function removeProduct(id) {
        $('#tableLoad').html('<center><img src=" {{asset('images/default/loader.gif')}}"/></center>')
            .load('{{URL::to("admin/remove-product-from-purchase-list?")}}'+'id='+id);
    }
    ...
    $('#discount').on('keyup',function () {
        ...
        var netTotalHidden = $('#netTotalHidden').val()
          $('#netTotal').html(netTotalHidden-discount)
    });
