@if(count($productPurchase->purchaseItem)>0)
<div class="table-responsive checkout-product" id="tableLoad">
    ...
    <table id="productList" class="table table-bordered table-hover" style="width:100%;">
        ...
        @foreach($productPurchase->purchaseItem as $product)
            <tr>
                ...
                <td>
                    {{$product->qty}}
                    <input type="hidden" name="invoice_item_qty[{{$product->id}}]" value="{{$product->qty}}" readonly>
                </td>

                <td>
                    <?php $balanceQty=($product->product->productStock->qty+$product->product->productStock->sold_return_qty)-($product->product->productStock->sold_qty+$product->product->productStock->purchase_return_qty)?>
                    {{$balanceQty}}
                    <input type="hidden" name="balance_item_qty[{{$product->id}}]" value="{{$balanceQty}}" readonly>
                </td>
                ...
                <td>{{$product->cost_price}}
                    <input type="hidden" name="cost_price[{{$product->id}}]" value="{{$product->cost_price}}" id="costPrice_{{$product->id}}">
                </td>
                ...
                <td><input type="number" name="item_return_price[{{$product->id}}]" value="0" id="itemTotalReturnPrice_{{$product->id}}" onkeyup="totalReturnAmountCalculation({{$product->id}})" class="itemTotalReturnPrice">
...
<div class="row">
    ...
    <div class="col-md-6 col-lg-6 payment-calculation">
        <table class="table table-bordered table-hover">
            ...
            <tr>
                ...
                <td class="text-left"><input type="number" name="vendor_total_due" value="{{$vendorTotalDue}}" id="vendorDue" class="form-control"></td>
            </tr>

            <tr>
                <td class="text-right"><strong>Return Amount :</strong></td>

                <td class="text-left"><input type="number" name="return_amount" value="" required id="totalReturnAmount" class="form-control"> </td>
            </tr>
    ...
    <div class="table-responsive checkout-product" id="tableLoad">
        ...
        <table id="productList" class="table table-bordered table-hover" style="width:100%;">
            ...
            <tbody>
            <tr>
                <td colspan="7" class="text-center text-danger">No Product Purchase Info Found !</td>
            </tr>
